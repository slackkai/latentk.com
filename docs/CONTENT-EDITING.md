...
五个内容板块使用文章式预览：标题、摘要、日期与作者集中在顶部，标签和类型/状态以小标签显示，正文接在后面。空的链接、评分、封面等不会占位；草稿、精选或关闭评论只显示简洁标记。修改左侧字段或正文后会实时更新，封面和相册使用 CMS 的附件预览地址，因此未保存的上传也能预览。

预览沿用网站的文楷字体、纸张背景、正文和笔记本扩展样式。窄预览栏中的页边批注就地显示，避免挤出栏外。它是内容预览，不包含网站导航、目录、相关文章和真实评论；交互嵌入仍以占位说明呈现，完整页面以部署后的站点或本地 Astro 预览为准。

模板在 `public/admin/previews.js`，专用布局在 `src/styles/cms-preview.css`；`npm run dev` / `npm run build` 会生成 `public/admin/preview.css` 并准备字体资源。
...
“站点设置 → 导航与评论”可以开关导航栏自动隐藏，并配置 giscus 评论。导航下滚隐藏、上滚显示；菜单展开、搜索和键盘操作时保持可见。

五个板块的每篇文章（含项目子文档）都有“允许评论”开关，默认开启；旧文章没有这个字段也默认开启。关闭仅隐藏该篇评论区，不删除已有 GitHub 讨论。全站开关关闭时，所有文章都不加载评论；草稿也不加载。首次接入、账号要求和迁移注意事项见 [评论配置](COMMENTS.md)。
