...
CMS 右侧的文章预览复用网站的公式、边注、脚注和手绘指令渲染规则，以及正文字体和样式。长公式可横向滚动；窄预览栏中点击批注编号可展开内容。正文中的图片通过 CMS 解析，支持文章附件与尚未提交的上传资源。

代码块复用网站的 Shiki 高亮、深浅色配色和换行规则，右上角显示语言标签。语言规则按需加载，短暂显示纯文本后自动着色；未知语言仍以纯文本显示。

预览边界：HTML 片段和嵌入页面显示占位说明，需发布后检查，后台不执行其中的脚本。右侧预览与左侧富文本编辑控件是不同的渲染界面。
